...
import {
	...
	withNavigation, Header, withUser, Message, Toast, FlatList, Alert
} from '../../../components';
...
export default class Attention extends React.Component {
			...
			[
				{ text: '取消', onPress: () => console.log('Cancel Pressed'), style: 'cancel' },
				{
					text: '确定', onPress: async () => {
						let res = await http.get(`/services/app/v1/attention/followApps/${checkItems}/0`);
						if (res.data.code === '200') {
							Toast.show({ text: '取消关注成功，您将不会再收到该资源相关消息~' });
							this.setState({
								animating: false,
								checkIds: {}
							});
			...
			<ListItem style={styles.mainLIst} onPress={() => this.checkBoxShow(item)} key={item.id}>
				...
				<View style={styles.LIst}>
					...
					<View style={styles.mainRight}>
						<Text style={[styles.mainRightItem, styles.mainRightItemTit]} numberOfLines={1}>{item.appliName}</Text>
						...
						<View style={styles.mainrTextRight}>
							...
							{
								this.state.animating ? null : <Text style={styles.attentionNum} note>{this.attentionNum(item.attentionNum)} 人关注</Text>
							}
	...
	async AttentionData() {
		let res = await http.get('/services/app/v1/attention/app/1/1000');
		if (res.data.code === '200') {
			if (res.data.data) {
