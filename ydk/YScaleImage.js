...
var Image = createReactClass({
	...
	render: function () {
		...
		if (source && (source.uri || Array.isArray(source))) {
			...
			const { onLoadStart, onLoad, onLoadEnd, onError,maxScale} = this.props;
			...
			if (nativeProps.children) {
				...
				return (
					<View style={containerStyle}>
						<YScaleImage {...imageProps} />
						{this.props.children}
					</View>
				);
			} else {
				return <YScaleImage {...nativeProps} />;
			}
		}
		return null;
	}
...
let YScaleImage = View
if (global.Expo) {
	Image = ReactImage;

} else {
	YScaleImage = requireNativeComponent('YScaleImage', Image, cfg);
}
